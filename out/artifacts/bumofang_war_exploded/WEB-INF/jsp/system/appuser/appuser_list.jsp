			...
			<table id="table_report" class="table table-hover">
				...
				<thead>
					<tr>
						...
						<th>姓名</th>
						<th>等级</th>
						<th><i class="fa fa-time hidden-phone"></i>到期日期</th>
						<th>年限</th>
						...
						<c:forEach items="${userList}" var="user" varStatus="vs">
							...
							<tr>
								<td class='center' style="width: 30px;">
									<label><input type='checkbox' class="ace" name='ids' value="${user.USER_ID }" id="${user.EMAIL }" alt="${user.PHONE }"/><span class="lbl"></span></label>
								</td>
								...
								<td>
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<button class="btn btn-mini btn-info" data-toggle="dropdown"><i class="fa fa-cog fa fa-only"></i></button>
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="editUser('${user.USER_ID }');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="fa fa-edit"></i></span></a></li>
											</c:if>
											<c:choose>
											<c:when test="${user.USERNAME=='admin'}"></c:when>
											<c:otherwise>
												<c:if test="${QX.del == 1 }">
												<li><a style="cursor:pointer;" title="删除" onclick="delUser('${user.USER_ID }','${user.USERNAME }');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="fa fa-trash"></i></span> </a></li>
												</c:if>
											</c:otherwise>
											</c:choose>
										</ul>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function sendEmail(EMAIL){
			 ...
			 BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>head/goSendEmail.do?EMAIL='+EMAIL+'&msg=appuser'),
	            title: '发送电子邮件'
	          });
		}
		...
		function add(){
			 BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>happuser/goAddU.do'),
	            title: '新增会员'
	          });
		}
		...
		function editUser(user_id){
			 BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<%=basePath%>happuser/goEditU.do?USER_ID='+user_id),
	            title: '会员资料'
	          });
		}
