...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid" style="padding:0 40px;">
				<div>
				<div style="float:right;">
					<button class="btn btn-mini btn-info" onclick="add();" style="padding:0 30px; margin:10px 50px;">新建账户</button>					
				</div> 
			</div>
			<!-- 检索  -->
...
<%-- 								<c:if test="${QX.FX_QX == 1 }">
								...
								<td>未登录</td>
								<td>${user.LAST_LOGIN_SHOW}</td>
								<%-- <td>${user.IP}</td> --%>
								<td>
									<div class='hidden-phone visible-desktop btn-group'>
										
										<c:if test="${QX.edit == 1 }">
											<c:if test="${user.USERNAME != 'admin'}"><a class='' style="cursor: pointer;"title="编辑" onclick="editUser('${user.USER_ID }');">编辑</a></c:if>
											<c:if test="${user.USERNAME == 'admin'}"><a class='btn btn-mini btn-info' title="您不能编辑">编辑</a></c:if>
										</c:if>
<%-- 										<c:choose>
...
<%-- 				<td style="vertical-align:top;">
					<c:if test="${QX.add == 1 }">
					<a class="btn btn-small btn-success" onclick="add();">新建账户</a>
					</c:if>
					<c:if test="${QX.del == 1 }">
