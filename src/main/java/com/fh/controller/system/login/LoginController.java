...
/*			if(null == code || "".equals(code)){
				...
				pd.put("USERNAME", loginName);
/*				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){*/
					String passwd = new SimpleHash("SHA-1", "bumofang", password).toString();	//密码加密
					...
					if(pd != null){
						...
						User user = new User();
						user.setUSER_ID(pd.getLong("USER_ID"));
					    ...
					    try { 
					        ...
					        sysLogBizService.saveLog(request, user,
									SysLogEnum.ACTION_LOGIN.getName(), SysLogEnum.ACTION_LOGIN.getContent());
...
/*				}else{
	...
	public ModelAndView logout(){
		...
		session.removeAttribute(Const.SESSION_NAME);
