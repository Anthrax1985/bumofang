...
public class MenuController extends BaseController {
	 ...
	 */
	  ...
	  @ResponseBody
	  public String restAdd(@RequestBody Map<String, String> map)
	    throws Exception
	  {
	    logBefore(this.logger, "REST新增Menu");
	    PageData pd = new PageData();
	    Menu menu = new Menu();
	    menu.setPARENT_ID(map.get("PARENT_ID"));
	    menu.setMENU_NAME(map.get("MENU_NAME"));
	    menu.setMENU_ORDER(map.get("MENU_ORDER"));
	    menu.setMENU_URL(map.get("MENU_URL"));
	    try{
			menu.setMENU_ID(String.valueOf(Integer.parseInt(menuService.findMaxId(pd).get("MID").toString())+1));
	  ...
	  public String restEdit(@RequestBody Map<String, String> map)
	    throws Exception
	  {
	    logBefore(this.logger, "REST编辑Menu");
	    PageData pd = new PageData();
		try{
			pd.putAll(map);
			...
			logger.warn(e.toString(), e);
			ResponseMessageEnum.SERVER_ERROR.toString();
		}
