...
public class UserController extends BaseController {
	...
	public ModelAndView goEditU() throws Exception{
		...
		//如果是P4，则无法创建P5的账号，即职级选择不展示P5
		User user = getCurrentUser();
		String userRoleId = user.getROLE_ID();
		if(userRoleId.equals("c51b63e4560e43f99a0afb9d364db388")){
			for(Role role: roleList ){
				if(role.getROLE_NAME().equals("P5")){
	...
	public ModelAndView listUsers(Page page)throws Exception{
		...
		for(PageData cycle: userList){
			String lastLogin = cycle.getString("LAST_LOGIN");
			...
			cycle.put("LAST_LOGIN_SHOW",dateInfo);
		}
		List<Role> roleList = roleService.listAllERRoles(page);						//列出所有二级角色
...
/*	@RequestMapping(value={"/rest/saveU"}, method={RequestMethod.POST}, produces={JSON_UTF8})
	@ResponseBody
			...
			throws Exception
	{
		...
		if(StringUtils.isBlank(userId)){
			//pd.put("USER_ID", this.get32UUID());	自增
			pd.put("EMAIL", "");	//邮箱暂时为空
			pd.put("PHONE", "");	//手机暂时为空
			pd.put("BZ", "");	//备注暂时为空
			pd.put("RIGHTS", "");					//权限
		...
		pd.put("PASSWORD", new SimpleHash("SHA-1", BMF, "12345678").toString());		
		PageData dbResult = userService.findByUId(pd);		
		if(dbResult != null){
			return ResponseMessageEnum.SERVER_DATA_REPEAT.toString();
		}		
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){
			ResponseMessageEnum.VISIT_NOT_AUTHORIZED.toString();
		} //判断新增权限		
		try{
			userService.saveU(pd);
			//日志
			...
			return ResponseMessageEnum.SERVER_ERROR.toString();
		}
		//返回结果
...
*/	  
	@RequestMapping(value={"/rest/saveU"}, method={RequestMethod.POST}, produces={JSON_UTF8})
	  ...
	  public String restSave(@RequestBody Map<String, String> map)throws Exception{
		...
		//一共三个注册账户信息，判断每个注册账户的账号，如果账号为空；则默认这账号不需要保存
		//重组数据
		List<PageData> saveList = new ArrayList<PageData>();
		for(int i = 0;i<3;i++){
			if(!map.get("USERNAME"+i).isEmpty()||!map.get("NAME"+i).isEmpty()||!map.get("JOB_NUMBER"+i).isEmpty()||!map.get("ROLE_ID"+i).isEmpty()){
				//保存第i个账户
				PageData saveInfo = new PageData();
				saveInfo.put("USERNAME", map.get("USERNAME"+i));	//账户账号
				saveInfo.put("NAME", map.get("NAME"+i));	//账户户主姓名
				saveInfo.put("JOB_NUMBER", map.get("JOB_NUMBER"+i));	//账户户主工号
				saveInfo.put("ROLE_ID", map.get("ROLE_ID"+i));	//账户角色
				saveList.add(saveInfo);
			}
		}
		Object result = userService.saveUserList(saveList, getCurrentUser());
	  ...
	  @RequestMapping(value={"/rest/hasU"}, method={RequestMethod.POST}, produces={JSON_UTF8})
	  @ResponseBody
	  public String restHasUser(@RequestBody Map<String, String> map)
	    throws Exception
	  {
		PageData pd = new PageData();
		try{
	  ...
	  @RequestMapping(value={"/rest/editU"}, method={RequestMethod.POST}, produces={JSON_UTF8})
	  @ResponseBody
...
/*		if(pd.getString("PASSWORD") != null && !"".equals(pd.getString("PASSWORD"))){
		...
		//重置密碼
		String formatPassword = map.get("formatPassword");
		if(formatPassword!=null && formatPassword!="" && formatPassword.equals("1")){
			pd.put("PASSWORD", new SimpleHash("SHA-1", BMF, "12345678").toString());
			logger.info("i want to modify password");
		}
		try{
			userService.editU(pd);
			//日志
			User user = getCurrentUser();
			PageData ueserInfo = userService.findByUiId(pd);
			sysLogBizService.saveLog(request, user,
					SysLogEnum.USER_EDIT_USER.getName(), SysLogEnum.USER_EDIT_USER.getContent()+":"+ueserInfo.getString("NAME"));
		}catch(Exception e){
			return ResponseMessageEnum.SERVER_ERROR.toString();
		}
				//返回结果
