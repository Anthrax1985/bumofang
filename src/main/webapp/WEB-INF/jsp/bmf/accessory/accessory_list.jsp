...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="accessory/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 BootstrapDialog.show({cssClass:'three-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>accessory/goAdd.do'),
	            title: '新增'
	          });
		}
		...
		function edit(id){
			BootstrapDialog.show({cssClass:'three-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>accessory/goEdit.do?ID='+id),
	            title: '编辑'
	          });
		}
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>accessory/delete.do?ID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>accessory/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>accessory/excel.do';
		}
