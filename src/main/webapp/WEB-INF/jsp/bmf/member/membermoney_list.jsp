...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="membermoney/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>membermoney/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>membermoney/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
		function del(id){
			var url = "<%=basePath%>membermoney/delete/"+id;
			ajaxDelete(url);
		}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>membermoney/deleteBatch';
		ajaxDeleteBatch(url);
	}
