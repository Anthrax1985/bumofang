						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 30px;">${vs.index+1}</td>
										<td>${var.name}</td>
										<td>${var.mobile}</td>
										<td>${var.addr_province}</td>
										<td>${var.addr_city}</td>
										<td>${var.addr_county}</td>
										<td>${var.addr_detail}</td>
										<td>${var.post_code}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 BootstrapDialog.show({cssClass:'two-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>receiver/goAdd.do'),
	            title: '新增'
	          });
		}
		...
		function edit(id){
			BootstrapDialog.show({cssClass:'two-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>receiver/goEdit.do?id='+id),
	            title: '编辑'
	          });
		}
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>receiver/delete.do?id="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>receiver/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>receiver/excel.do';
		}
