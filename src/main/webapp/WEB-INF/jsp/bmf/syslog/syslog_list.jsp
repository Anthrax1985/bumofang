...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="syslog/list.do" method="post" name="Form" id="Form">
<%-- 			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-hover" style="margin-left:20px;table-layout: fixed;">
...
<%-- 								<td style="width: 30px;">
								...
								<td >${var.createTimeShow}</td>
										<td>${var.userName}</td>
										<td>${var.roleType}</td>
										<td width="50%" style="overflow:hidden">${var.pageName}：${var.operationContent}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>syslog/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>syslog/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
		function del(id){
			var url = "<%=basePath%>syslog/delete/"+id;
			ajaxDelete(url);
		}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>syslog/deleteBatch';
		ajaxDeleteBatch(url);
	}
