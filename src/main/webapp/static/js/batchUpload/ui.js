...
FileProgress.prototype.setComplete = function(up, info) {
  ...
  if (res.url) {
    ...
    var imgProductDomain = up.getOption('imgProductDomain');
    var imgD3dDomain = up.getOption('imgD3dDomain');
    var imgSceneDomain = up.getOption('imgSceneDomain');

    var baseUrl = "";
    var fileKey = res.key;
    var arr = fileKey.split("_");

    if(fileKey.substr(0,3) == 'FAH') {
        baseUrl = imgProductDomain;
    }else if(arr.length == 4){
        baseUrl = imgD3dDomain;
    }else{
        baseUrl = imgSceneDomain;
    }

    url = baseUrl + encodeURI(res.key);
    var link = baseUrl + res.key;
