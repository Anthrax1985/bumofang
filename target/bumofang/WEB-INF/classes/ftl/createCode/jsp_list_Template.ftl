				...
				<tr>
					<td>
						<span class="input-icon">
							<input autocomplete="off" id="nav-search-input" type="text" name="QUERY_KEY" value="${r"${pd.QUERY_KEY}"}" placeholder="这里输入关键词" />
							<i class="ace-icon fa fa-search nav-search-icon"></i>
						</span>
					</td>
					<td><input class="span10 date-picker" name="QUERY_TIME_START" id="QUERY_TIME_START" value="${r"${pd.QUERY_TIME_START}"}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="开始日期"/></td>
					<td><input class="span10 date-picker" name="QUERY_TIME_END" id="QUERY_TIME_END" value="${r"${pd.QUERY_TIME_END}"}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期"/></td>
				...
				<c:choose>
					...
					<c:when test="${r"${not empty varList}"}">
						...
						<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
							<tr>
								<td style="width: 30px;">
									<label><input type='checkbox' name='ids' class="ace" value="${r"${var."}id${r"}"}" /><span class="lbl"></span></label>
								</td>
								<td style="width: 30px;">${r"${vs.index+1}"}</td>
								<#list fieldList as var>
										<td>${r"${var."}${var[0]}${r"}"}</td>
								</#list>
									...
									<div>
										...
										<div class="inline position-relative">
											<c:if test="${r"${QX.edit == 1 }"}">
												<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${r"${var."}id${r"}"}');"><i class="fa fa-edit"></i></a>
											</c:if>
											<c:if test="${r"${QX.del == 1 }"}">
												<a class='btn btn-mini btn-danger' title="删除" onclick="del('${r"${var."}id${r"}"}');"><i class="fa fa-trash"></i></a>
											</c:if>
										</ul>
										</div>
									</div>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 BootstrapDialog.show({cssClass:'two-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>${objectNameLower}/goAdd.do'),
	            title: '新增'
	          });
		}
		...
		function edit(id){
			BootstrapDialog.show({cssClass:'two-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>${objectNameLower}/goEdit.do?id='+id),
	            title: '编辑'
	          });
		}
