...
<div class="container-fluid" id="main-container">
	...
	<div class="selectTime">
		<form action="productdatastatistics/listForData.do" method="post" name="Form" id="Form">
	...
	<div>
		<ul id="overviewUl">
			...
			<li>总下单次数：${resInfo.totalOrderCnt}次</li>
			<li>总支付次数：${resInfo.totalPayCnt}次</li>
			<li>转换率:<fmt:formatNumber type="number" value="${resInfo.totalPayCnt/resInfo.totalBrowseAmount}" pattern="0.00"/>%</li>
			<li>总下单米数：${resInfo.totalMeter}米</li>
			<li>总下单金额：${resInfo.totalPrice}元</li>
			<li>总支付金额：${resInfo.totalRealPay}元</li>
			<!-- <li>充值：0元</li> -->
		</ul>
	</div>
	<div>
					...
					<c:when test="${not empty resInfo.browseAmountList}">
						<c:forEach items="${resInfo.browseAmountList}" var="list">
							<tr>
								...
								<td>${list.orderCnt}</td>
								<td>${list.payCnt}</td>
								<td><fmt:formatNumber type="number" value="${list.payCnt/list.browseAmount}" pattern="0.00"/>%</td>
								<td>${list.orderMeter}</td>
...
<script type="text/javascript">	
	...
	$(".tableDetail").click(function openTableDetail(){
				...
				<%-- window.open('<%=basePath%>productdatastatistics/tableList/'+status+'/'+periodTime+'/'+productId+'/'+memberId,"新增","width=1080,height=480,screenX=300,screenY=100, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no")  --%>
				layer.open({
					  type: 2, 
					  title:"浏览详情",
					  area: ['1080px', '600px'],
