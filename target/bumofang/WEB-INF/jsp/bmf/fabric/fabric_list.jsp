...
<html lang="en">
	...
	<script src="<%=basePath%>static/js/bmf/vue.js"></script>
	</head>
<body>
...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="fabric/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
							...
							<tr>
								<td style="width: 30px;">
									<label><input type='checkbox' name='ids' class="ace" value="${var.ID}" /><span class="lbl"></span></label>
										...
										<td>${var.NAME}</td>
										<td>${var.COLOR}</td>
										<td>${var.INGREDIENTS}</td>
										<td>${var.WIDTH}</td>
										<td>${var.FLOWER_SIZE}</td>
										<td>${var.FLOWER_DISTANCE}</td>
										<td>${var.WEIGHT}</td>
										<td><img src="<%=basePath %>${var.FLAT_PICTURE}" height="80"></td>
										<td><img src="<%=basePath %>${var.ART_PICTURE}" height="80"></td>
										<td><img src="<%=basePath %>${var.STREAM_PICTURE}" height="80"></td>
										<td>${var.PRICE}</td>
										<td>${var.RECORD_DATE}</td>
								<td style="width: 100px;" class="center">
									<div>
										...
										<div class="inline position-relative">
											<c:if test="${QX.edit == 1 }">
<%-- 												<a id="edit_status" class='btn btn-mini btn-success' show="${var.IS_SHOW }" onclick="editStatus(this, '${var.ID}');"> --%>
<!-- 													<i class="fa "></i></a> -->
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 BootstrapDialog.show({cssClass:'three-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>fabric/goAdd.do'),
	            title: '新增'
	          });
		}
		...
		function edit(id){
			BootstrapDialog.show({cssClass:'three-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>fabric/goEdit.do?ID='+id),
	            title: '编辑'
	          });
		}
		...
		function editStatus(id, status){
			var ajaxObject = {};
			ajaxObject.url = "<%=basePath%>fabric/rest/editStatus";
			ajaxObject.data = {ID:id, STATUS: status};
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>fabric/delete.do?ID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>fabric/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>fabric/excel.do';
		}
