...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="messagemember/list.do" method="post" name="Form" id="Form">
<%-- 			<table>
				<tr>
					<td>
...
<%-- 								<td style="width: 30px;">
								...
								<td style="width: 40px;">${vs.index+1}</td> --%>
										...
										<td>${var.memberAvatar}</td>
										<td>${var.contentInfo}</td>
										<td>${var.contentTime}</td>
										<td>${var.replyInfo}</td>
										<td>${var.replyTime}</td>
										<td>${var.replierId}</td>
										<td>${var.replierName}</td>
										<td>${var.replierAvatar}</td>
										<td>${var.status}</td>
										<td>${var.delFlag}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>messagemember/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>messagemember/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
		function del(id){
			var url = "<%=basePath%>messagemember/delete/"+id;
			ajaxDelete(url);
		}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>messagemember/deleteBatch';
		ajaxDeleteBatch(url);
	}
