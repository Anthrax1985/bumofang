...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="productparammaterial/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 40px;">${vs.index+1}</td>
										<td>${var.materialName}</td>
										<td>${var.materialOrder}</td>
...
<script type="text/javascript">
	...
	function add(){
		 BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>productparammaterial/goAdd.do'),
            title: '新增'
          });
	}
	...
	function edit(id){
		BootstrapDialog.show({cssClass:'two-row-dialog',
            message: $('<div></div>').load('<%=basePath%>productparammaterial/goEdit/'+id),
            title: '编辑'
          });
	}
	...
	//删除
		function del(id){
			var url = "<%=basePath%>productparammaterial/delete/"+id;
			ajaxDelete(url);
		}
...
<script type="text/javascript">
	...
	//批量操作
	function makeAll(){
		var url = '<%=basePath%>productparammaterial/deleteBatch';
		ajaxDeleteBatch(url);
	}
