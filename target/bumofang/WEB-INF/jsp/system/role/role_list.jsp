  ...
  <div class="row-fluid">
	...
	<div id="breadcrumbs">
				...
				<td style="width:69px;"><a href="javascript:addRole();" class="btn btn-small btn-success">新增</a></td>
					...
					<c:forEach items="${roleList}" var="role" varStatus="vs">
						<td style="width:100px;" class="center" <c:choose><c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#FFC926';" >
							<a href="role.do?ROLE_ID=${role.ROLE_ID }" style="text-decoration:none; display:block;"><li class=" fa fa-group"></li>&nbsp;<font color="#666666">${role.ROLE_NAME }</font></a>
						</td>
		...
		<table>
			...
			<td>
			<c:if test="${QX.edit == 1 }">
			<a class="btn btn-mini btn-info" onclick="editRole('${pd.ROLE_ID }');">修改组名称<i class="fa fa-arrow-right  fa fa-on-right"></i></a>
			</c:if>
				<c:choose>
					...
					<c:otherwise>
					<c:if test="${QX.edit == 1 }">
					<a class="btn btn-mini btn-purple" onclick="editRights('${pd.ROLE_ID }');"><i class="fa fa-pencil"></i>组菜单权限</a>
					</c:if>
					</c:otherwise>
				</c:choose>
				<c:choose> 
					...
					<c:otherwise>
					 <c:if test="${QX.del == 1 }">
					 <a class='btn btn-mini btn-danger' title="删除" onclick="delRole('${pd.ROLE_ID }','z','${pd.ROLE_NAME }');"><i class='fa fa-trash'></i></a>
					 </c:if>
					</c:otherwise>
				</c:choose>
			</td>
	...
	<table id="table_report" class="table table-hover">
		...
		<c:choose>
			<c:when test="${not empty roleList_z}">
				...
				<td style="width:30px;"><a onclick="roleButton('${var.ROLE_ID }','add_qx');" class="btn btn-warning btn-mini" title="分配新增权限"><i class="fa fa-wrench fa fa fa-only"></i></a></td>
				<td style="width:30px;"><a onclick="roleButton('${var.ROLE_ID }','del_qx');" class="btn btn-warning btn-mini" title="分配删除权限"><i class="fa fa-wrench fa fa fa-only"></i></a></td>
				<td style="width:30px;"><a onclick="roleButton('${var.ROLE_ID }','edit_qx');" class="btn btn-warning btn-mini" title="分配修改权限"><i class="fa fa-wrench fa fa fa-only"></i></a></td>
				<td style="width:30px;"><a onclick="roleButton('${var.ROLE_ID }','cha_qx');" class="btn btn-warning btn-mini" title="分配查看权限"><i class="fa fa-wrench fa fa fa-only"></i></a></td>
				</c:if>
				...
				<c:if test="${QX.edit == 1 }">
				<a class="btn btn-mini btn-purple" onclick="editRights('${var.ROLE_ID }');"><i class="fa fa-pencil"></i>菜单权限</a>
				<a class='btn btn-mini btn-info' title="编辑" onclick="editRole('${var.ROLE_ID }');"><i class='fa fa-edit'></i></a>
				</c:if>
				<c:choose> 
					...
					<c:otherwise>
					 <c:if test="${QX.del == 1 }">
					 <a class='btn btn-mini btn-danger' title="删除" onclick="delRole('${var.ROLE_ID }','c','${var.ROLE_NAME }');"><i class='fa fa-trash'></i></a>
					 </c:if>
					</c:otherwise>
				</c:choose>
				</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function addRole(){
			 BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>role/toAdd.do?parent_id=0'),
	            title: '新增组'
	          });
		}
		...
		function addRole2(pid){
			BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>role/toAdd.do?parent_id='+pid),
	            title: '新增角色'
	          });
		}
		...
		function editRole(ROLE_ID){
			BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<div style="width:200px;"></div>').load('<%=basePath%>role/toEdit.do?ROLE_ID='+ROLE_ID),
	            title: '编辑'
	          });
		}
		...
		function editRights(ROLE_ID){
			BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>role/auth.do?ROLE_ID='+ROLE_ID),
	            title: '菜单权限'
	          });
		...
		function roleButton(ROLE_ID,msg){
			if(msg == 'add_qx'){
			 ...
			 BootstrapDialog.show({cssClass:'one-row-dialog',
	            message: $('<div></div>').load('<%=basePath%>role/button.do?ROLE_ID='+ROLE_ID+'&msg='+msg),
	            title: Title
	          });
		}
